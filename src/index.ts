...
const server = new McpServer({
  name: "BGG API Explorer Server", // サーバーの名前 [j]
  version: "1.0.0", // サーバーのバージョン [j]
});

// BGG API の Thing コマンドに対応する MCP Resource を定義
// 例: bgg://thing/{id} の形式でリソースにアクセスすることを想定
// {id} は BGG の Thing ID (ゲーム、拡張、アクセサリなど) [i]
server.resource(
  "bgg-thing", // MCP Resource の名前
  new ResourceTemplate("bgg://thing/{id}", { list: undefined }), // Resource の URI パターン [j]
  async (uri, { id }) => {
    // Resource が呼び出されたときのハンドラー関数 [j]
    console.log(`Received request for BGG Thing ID: ${id}`);

    try {
      // 資料 [i] に記載されている BGG API のルートパスと Thing コマンドのベース URI を使用して URL を構築
      // 例として stats=1 パラメータを追加し、評価/ランキング情報も取得 [i]
      ...
      // 抽出したデータを MCP Resource のレスポンス形式に合わせて整形 [j]
      // MCP Resource は typically text コンテンツを返します [j]
      ...
      return {
        contents: [
          {
            uri: uri.href,
            text: contentText, // テキスト形式のコンテンツを返す [j]
          },
        ],
      };
    } catch (error) {
      console.error(`Error processing BGG Thing ID ${id}:`, error);
      // エラーが発生した場合は、エラー情報を含むレスポンスを返す [j]
      return {
        contents: [],
        isError: true,
        errorMessage: `Failed to retrieve BGG data for ID ${id}: ${error.message}`,
      };
    }
  }
);
function parseStringPromise(xmlText: string, arg1: { explicitArray: boolean; }) {
